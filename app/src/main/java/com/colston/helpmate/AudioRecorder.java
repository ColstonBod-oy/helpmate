...
public class AudioRecorder {
  ...
  public void start() {
    if (isRecording()) {
      Log.w(Constants.TAG, "Already running");
      return;
    }
        ...
        new Thread() {
          ...
          public void run() {
            ...
            if (record.getState() != AudioRecord.STATE_INITIALIZED) {
              Log.w(Constants.TAG, "Failed to start recording");
              mAlive = false;
              return;
            }
            ...
            try {
              while (isRecording()) {
                ...
                if (len >= 0 && len <= buffer.size) {
                  ...
                  Log.w(Constants.TAG, "Unexpected length returned: " + len);
                }
              }
            } catch (IOException e) {
              Log.e(Constants.TAG, "Exception with recording stream", e);
            } finally {
              stopInternal();
              try {
                record.stop();
              } catch (IllegalStateException e) {
                Log.e(Constants.TAG, "Failed to stop AudioRecord", e);
              }
              record.release();
            }
          }
  ...
  private void stopInternal() {
    ...
    try {
      ...
      Log.e(Constants.TAG, "Failed to close output stream", e);
    }
  }
  ...
  public void stop() {
    ...
    try {
      ...
      Log.e(Constants.TAG, "Interrupted while joining AudioRecorder thread", e);
